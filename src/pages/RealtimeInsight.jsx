...
export function RealtimeInsight(){
    ...
    useEffect(() => {
        if(!isNotificationStart){
            ...
            const GetNotificationAlert = async() => {
                ...
                .then((data) => {
                    responseData = data; 
                    setNotificationData(responseData.rows)
                });
            }
    ...
    const handleCloseNotification = (info) => {
        const requestOptions = {
            method: 'DELETE',
            headers: { 
                'Content-Type': 'application/json',
        ...
        .then((data) => {
            if(data.ok){
                let updatedRows = [...notificationData]
                var indexToRemove = notificationData.findIndex(x => x.value._id === info.value._id);
                if(indexToRemove > -1){
                        ...
                        <Grid item xs={12}>
                            <Paper className={classes.paper}>
                                ...
                                {notificationData.map((data,index) => (
                                    <Alert className={classes.alertMessage} variant="filled"  elevation={3} key={index} severity="warning"  onClose={() => handleCloseNotification(data)}>Alert for {data.value.deviceId}, Temperature is : {data.value.temperature}°C and Humidity is : {data.value.humidity}%</Alert>
                                    ))
                                }
                            </Paper>
                        </Grid>
